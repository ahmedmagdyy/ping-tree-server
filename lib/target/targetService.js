...
async function addNewTarget (target) {
  ...
  try {
    ...
    await setTargetExpiration(target.id)
...
async function setTargetExpiration (targetId) {
  try {
    // create expired hash for max accepts per day
    await redisClient.hmset(targetMaxAcceptsPerDayKey, targetId, 0)
    // expires in 1 day
    await redisClient.expire(targetMaxAcceptsPerDayKey, 1 * 60)
  } catch (error) {
    console.log(error)
    return error
  }
